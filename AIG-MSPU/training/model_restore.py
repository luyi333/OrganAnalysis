...
def restore_model(pkl_file, checkpoint=None, train=False, fp16=None):
    ...
    init = info['init']
    name = info['name']
    search_in = join(AIG_MSPU.__path__[0], "training", "network_training")
    tr = recursive_find_python_class([search_in], name, current_module="AIG_MSPU.training.network_training")
    ...
    if tr is None:
        raise RuntimeError("Could not find the model trainer specified in checkpoint in AIG_MSPU.trainig.network_training. If it "
                           "is not located there, please move it or change the code of restore_model. Your model "
                           "trainer can be located in any directory within AIG_MSPU.trainig.network_training (search is recursive)."
