...
from AIG_MSPU.inference.segmentation_export import save_segmentation_nifti_from_softmax
from AIG_MSPU.network_architecture.generic_UNet import Generic_UNet
...
from AIG_MSPU.training.dataloading.dataset_loading import load_dataset, DataLoader3D, DataLoader2D, unpack_dataset
from AIG_MSPU.training.loss_functions.dice_loss import DC_and_CE_loss
from AIG_MSPU.training.network_training.network_trainer import NetworkTrainer
from AIG_MSPU.utilities.nd_softmax import softmax_helper
from AIG_MSPU.utilities.tensor_utilities import sum_tensor

from AIG_MSPU.training.dataloading.data_loading_presave_prob import divide_prob
...
class nnUNetTrainer(NetworkTrainer):
    ...
    def preprocess_patient(self, input_files):
        ...
        preprocessor_class = recursive_find_python_class([join(AIG_MSPU.__path__[0], "preprocessing")],
                                                         preprocessor_name,
                                                         current_module="AIG_MSPU.preprocessing")
        assert preprocessor_class is not None, "Could not find preprocessor %s in AIG_MSPU.preprocessing" % \
                                               preprocessor_name
    ...
    def validate(self, do_mirroring: bool = True, use_sliding_window: bool = True, step_size: float = 0.5,
        ...
        task_dir = ''
        for i in range(1, len(self.folder_with_preprocessed_data.split('/'))-1):
            task_dir += '/' + self.folder_with_preprocessed_data.split('/')[i]
        print('task_dir', task_dir)
        prob_path = join(task_dir, 'prob_maps', 'prob_fusion_%d.npy'%self.fold)
        print('prob_path', prob_path)
        prob_fusion = np.load(prob_path, mmap_mode='r')
        print('prob_fusion.shape', prob_fusion.shape)
        # print('self.num_classes', self.num_classes)
        # prob_divide = divide_prob(prob_fusion, nc=self.num_classes-1)
        # print('prob_divide.shape', prob_divide.shape)
        ...
        for k in self.dataset_val.keys():
            ...
            if overwrite or (not isfile(join(output_folder, fname + ".nii.gz"))) or \
                ...
                factor = np.array(data[:-1].shape[1:]) / np.array(prob_fusion.shape)
                print('factor', factor)
                prob_fusion_this_case = zoom(prob_fusion, factor, order=0)
                print('prob_fusion_this_case.shape', prob_fusion_this_case.shape)
                prob_divide_this_case = divide_prob(prob_fusion_this_case, nc=self.num_classes-1)
                print('prob_divide_this_case.shape', prob_divide_this_case.shape)

                ##########!!!!!!!!!!!!!!
                ########## / 9 !!!!!!!!!
                prob_divide_this_case = prob_divide_this_case / 9

                data_for_net = np.concatenate((data[:-1], prob_divide_this_case), axis=0)
                print('data_for_net.shape', data_for_net.shape)
